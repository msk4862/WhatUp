...
exports.createNotifictionOnLikeHandler = (likeDoc) => {
    return db.doc(`/posts/${likeDoc.data().postId}`).get()
        .then(post => {
            // sender !== receiver
            if(!post.exists && post.data().userHandle !== likeDoc.data().userHandle) {
...
exports.createNotifictionOnCommentHandler = (commentDoc) => {
    return db.doc(`/posts/${commentDoc.data().postId}`).get()
        .then(post => {
            // sender !== receiver
            if(post.exists && post.data().userHandle === commentDoc.data().userHandle) {
...
exports.onUserImageChangeHandler = (change) => {
    if(change.before.data().imageUrl !== change.after.data().imageUrl) {
        ...
        return db.collection("posts")
            ...
            .then(posts => {
                posts.forEach(post => {
                    batch.update(post, {userImage: change.after.data().userImage});
                })
